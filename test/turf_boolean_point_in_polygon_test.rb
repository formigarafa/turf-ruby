...
class TurfBooleanPointInPolygonTest < Minitest::Test
  ...
  def test_feature_collection
    ...
    concave_poly = Turf.polygon([[[0, 0], [50, 50], [0, 100], [100, 100], [100, 0], [0, 0]]])
  ...
  def test_poly_with_hole
    ...
    assert_equal(Turf.boolean_point_in_polygon(pt_outside_poly, poly_hole), false)
  end
  ...
  def test_multipolygon_with_hole
    ...
    multi_poly_hole = load_geojson("boolean_point_in_polygon/multipoly-with-hole.geojson")

    assert_equal(Turf.boolean_point_in_polygon(pt_in_hole, multi_poly_hole), false)
    assert_equal(Turf.boolean_point_in_polygon(pt_in_poly, multi_poly_hole), true)
    assert_equal(Turf.boolean_point_in_polygon(pt_in_poly2, multi_poly_hole), true)
    assert_equal(Turf.boolean_point_in_polygon(pt_in_poly, multi_poly_hole), true)
    assert_equal(Turf.boolean_point_in_polygon(pt_outside_poly, multi_poly_hole), false)
  end
  ...
  def test_boundary_test
    ...
    [true, false].each do |ignore_boundary|
      ...
      tests.each_with_index do |test, test_index|
        ...
        assert_equal(
          Turf.boolean_point_in_polygon(point, poly, ignore_boundary: ignore_boundary),
          expect_result,
          "Boundary #{test_index}",
        )
      end
